    ...
    def make_instance(self, include_optional) -> GetAccount200Response:
        ...
        if include_optional:
            return GetAccount200Response(
                account = {team_name=Acme Corp, plan=pro, sandbox_mode=false, api_key_name=production-key, api_key_sandbox=false, limits={requests_per_month=1000, signing_requests_per_month=-1, max_templates=-1, max_members=10}, usage={requests=342, signing_requests=15, templates=3, members=2}}
            )
        else:
            return GetAccount200Response(
                account = {team_name=Acme Corp, plan=pro, sandbox_mode=false, api_key_name=production-key, api_key_sandbox=false, limits={requests_per_month=1000, signing_requests_per_month=-1, max_templates=-1, max_members=10}, usage={requests=342, signing_requests=15, templates=3, members=2}},
