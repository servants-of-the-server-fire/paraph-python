...
class CreateRequestRequest(BaseModel):
    ...
    """ # noqa: E501
    template_id: UUID = Field(description="ID of the template to fill")
    fields: Optional[Dict[str, StrictStr]] = Field(default=None, description="Map of field names to values. Text fields accept strings. Checkbox fields accept \"checked\" or \"unchecked\".")
    signers: Optional[Dict[str, SignerInput]] = Field(default=None, description="Map of signer label to signer configuration. Each key is a signer label defined in the template's signature placements. Omit entirely for fill-only requests. ")
    title: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(default=None, description="Display title for the request. Defaults to the template name. Shown in emails and dashboards.")
    message: Optional[Annotated[str, Field(strict=True, max_length=1024)]] = Field(default=None, description="Custom message included in signing emails sent to signers.")
    metadata: Optional[Dict[str, Annotated[str, Field(strict=True, max_length=1024)]]] = Field(default=None, description="Arbitrary key-value pairs for your own use (max 10 keys, key max 128 chars, value max 1024 chars)")
    allow_typed_signature: Optional[StrictBool] = Field(default=True, description="Whether signers can type their name as a signature instead of drawing or uploading one. Defaults to true. ")
    __properties: ClassVar[List[str]] = ["template_id", "fields", "signers", "title", "message", "metadata", "allow_typed_signature"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each value in signers (dict)
        _field_dict = {}
        if self.signers:
            for _key_signers in self.signers:
                if self.signers[_key_signers]:
                    _field_dict[_key_signers] = self.signers[_key_signers].to_dict()
            _dict['signers'] = _field_dict
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "signers": dict(
                (_k, SignerInput.from_dict(_v))
                for _k, _v in obj["signers"].items()
            )
            ...
            "allow_typed_signature": obj.get("allow_typed_signature") if obj.get("allow_typed_signature") is not None else True
        })
        return _obj
