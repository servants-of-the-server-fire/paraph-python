...
class ModelField(BaseModel):
    ...
    """ # noqa: E501
    id: UUID
    name: StrictStr = Field(description="Field name as defined in the PDF form")
    type: StrictStr = Field(description="AcroForm field type")
    options: List[StrictStr] = Field(description="Available options for ComboBox, ListBox, or RadioButtonGroup fields")
    __properties: ClassVar[List[str]] = ["id", "name", "type", "options"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['text', 'date', 'checkbox', 'combobox', 'listbox', 'radio_group']):
            raise ValueError("must be one of enum values ('text', 'date', 'checkbox', 'combobox', 'listbox', 'radio_group')")
        return value
