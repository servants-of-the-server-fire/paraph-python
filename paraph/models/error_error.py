...
class ErrorError(BaseModel):
    ...
    error_msg: StrictStr
    __properties: ClassVar[List[str]] = ["error_name", "error_msg"]

    @field_validator('error_name')
    def error_name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['unauthorized', 'not_found', 'bad_request', 'exceeded_quota', 'exceeded_rate', 'unknown', 'merge_failed', 'invalid_status']):
            raise ValueError("must be one of enum values ('unauthorized', 'not_found', 'bad_request', 'exceeded_quota', 'exceeded_rate', 'unknown', 'merge_failed', 'invalid_status')")
        return value
