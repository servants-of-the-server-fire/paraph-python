...
class UpdateTemplateRequest(BaseModel):
    ...
    """ # noqa: E501
    name: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(default=None, description="New template name (omit to leave unchanged)")
    metadata: Optional[Dict[str, Annotated[str, Field(strict=True, max_length=1024)]]] = Field(default=None, description="Replace all metadata (omit to leave unchanged, key max 128 chars, value max 1024 chars)")
    signature_placements: Optional[List[SignaturePlacementInput]] = Field(default=None, description="Replace all signature placements (omit to leave unchanged)")
    __properties: ClassVar[List[str]] = ["name", "metadata", "signature_placements"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in signature_placements (list)
        _items = []
        if self.signature_placements:
            for _item_signature_placements in self.signature_placements:
                if _item_signature_placements:
                    _items.append(_item_signature_placements.to_dict())
            _dict['signature_placements'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "signature_placements": [SignaturePlacementInput.from_dict(_item) for _item in obj["signature_placements"]] if obj.get("signature_placements") is not None else None
        })
        return _obj
